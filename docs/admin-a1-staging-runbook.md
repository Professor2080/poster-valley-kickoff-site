...
`product_registry.product_code` is immutable lowercase kebab-case. It seeds `eurofighter-typhoon-a2` as active `custom_drop`. Optional Woo references are permitted only for `woocommerce` mode; A1 does not contact WooCommerce. The registry does not become an editable price/stock/order authority: `api/_drops.js` and server quote logic remain authoritative for custom pricing until a separately approved parity migration.
