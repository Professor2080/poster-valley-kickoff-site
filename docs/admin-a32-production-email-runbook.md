...
Preview and Clean Staging always suppress operational invitation mail. Legacy Staging is inactive,
...
1. Validate the A3.2 contract as part of the canonical
   ...
   applied independently. Existing Production requires a separately approved read-only
   compatibility check and additive DDL; never run the initial baseline DDL there.
