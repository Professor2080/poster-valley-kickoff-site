...
test('contextual actions respect record lifecycle and manager-only quote controls', () => {
  ...
  assert.deepEqual(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'shipped', shipping_email_status: 'failed' }, 'operator'), [])
  assert.equal(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'shipped', shipping_email_status: 'failed' }, 'manager')[0].mutationAction, 'shipping.retry')
  assert.deepEqual(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'shipped', shipping_email_status: 'pending', shipping_reconciliation_required: true }, 'operator'), [])
  assert.equal(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'shipped', shipping_email_status: 'pending', shipping_reconciliation_required: true }, 'manager')[0].mutationAction, 'shipping.reconciliation.resolve')
  assert.deepEqual(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'shipped', shipping_email_status: 'pending', shipping_reconciliation_required: false }, 'manager'), [])
  ...
  assert.deepEqual(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'packed' }, 'operator'), [])
  assert.equal(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'packed' }, 'manager')[0].targetStatus, 'shipped')
  assert.equal(actions.contextualActions('orders', { id, status: 'paid', fulfilment_status: 'ready_to_pack' }, 'operator')[0].targetStatus, 'packed')
})
...
test('action feedback distinguishes truthful delivery outcomes and actionable errors', () => {
  ...
  assert.match(actions.actionResultMessage({ success: true, deliveryStatus: 'sent' }), /provider accepted/i)
  assert.match(actions.actionResultMessage({ success: true, deliveryStatus: 'pending', reconciliationRequired: true }), /uncertain/i)
  assert.match(actions.actionResultMessage({ success: true, deliveryStatus: 'pending', reconciliationRequired: true }), /not automatically resent/i)
  assert.match(actions.actionResultMessage({ success: true, deliveryStatus: 'sent', reconciliationOutcome: 'provider_acceptance_confirmed' }), /not inbox delivery/i)
  assert.match(actions.actionResultMessage({ success: true, deliveryStatus: 'failed', reconciliationOutcome: 'provider_non_acceptance_confirmed' }), /without sending/i)
  ...
  assert.equal(actions.completionPhase({ success: true, deliveryStatus: 'failed', reconciliationOutcome: 'provider_non_acceptance_confirmed' }), 'success')
...
test('Admin UI uses contextual preview-confirm controls and accessible outcome focus', async () => {
  ...
  assert.match(app, /Provider outcome/)
  assert.match(app, /Evidence note/)
  assert.match(app, /Do not include customer contact, address, provider IDs, secrets or tokens/)
  assert.match(app, /result\.deliveryStatus === 'pending' && result\.reconciliationRequired[\s\S]*actionResultMessage\(result\)/)
