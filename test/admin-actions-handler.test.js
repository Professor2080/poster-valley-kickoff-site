...
const oldFetch = globalThis.fetch; const oldUrl = process.env.SUPABASE_URL; const oldKey = process.env.SUPABASE_SERVICE_ROLE_KEY; const oldConfirmation = process.env.ADMIN_CONFIRMATION_SECRET
...
let rpcCalls = []
before(() => { globalThis.fetch = async (url, init = {}) => { const path = new URL(url).pathname; if (path === '/auth/v1/user') return new Response(JSON.stringify({ id: 'aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa' })); if (path.endsWith('/admin_roles')) return new Response(JSON.stringify([{ role: 'manager' }])); if (path.includes('/rpc/')) { rpcCalls.push({ path, body: JSON.parse(init.body) }); return new Response(JSON.stringify({ success: true, preview: { reservationId: 'aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa', suggestedAction: 'invitation.send', maskedRecipient: 'f***@example.test' } })) } throw new Error(path) } })
after(() => { globalThis.fetch = oldFetch; if (oldUrl === undefined) delete process.env.SUPABASE_URL; else process.env.SUPABASE_URL = oldUrl; if (oldKey === undefined) delete process.env.SUPABASE_SERVICE_ROLE_KEY; else process.env.SUPABASE_SERVICE_ROLE_KEY = oldKey; if (oldConfirmation === undefined) delete process.env.ADMIN_CONFIRMATION_SECRET; else process.env.ADMIN_CONFIRMATION_SECRET = oldConfirmation })
...
test('actual handler rejects mutation without explicit confirmation before RPC', async () => { rpcCalls=[]; const res=response(); await handler(request({ action:'invitation.send',reservationId:'aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa',idempotencyKey:'fixture-key' }),res); assert.equal(res.statusCode,409); assert.equal(rpcCalls.length,0) })
