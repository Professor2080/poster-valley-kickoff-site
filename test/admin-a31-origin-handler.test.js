...
beforeEach(() => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (name === 'admin_a31_change_origin') {
...
test('origin change requires explicit confirmation and keeps a stable idempotent replay', async () => {
  ...
  const first = response(); await handler(request({ ...payload, confirmation: 'CONFIRM' }), first)
  const replay = response(); await handler(request({ ...payload, confirmation: 'CONFIRM' }), replay)
  ...
  assert.equal(rpcCalls.filter((call) => call.name === 'admin_a31_change_origin').length, 1)
})

test('same origin idempotency key with changed payload is rejected as a conflict', async () => {
  const base = { action: 'origin.change', reservationId, expectedOriginVersion: 2, reason: 'Reserved .test fixture.', idempotencyKey: 'origin-fixture-002', confirmation: 'CONFIRM' }
  const first = response(); await handler(request({ ...base, recordOrigin: 'test' }), first)
  const conflict = response(); await handler(request({ ...base, recordOrigin: 'internal_pilot' }), conflict)
