...
function ReportResults({ report }: { report: AdminReportResponse }) {
  ...
  return <div className="admin-report-results">
    ...
    <div className="admin-report-queues"><Queue title="Fulfilment operations queue" rows={report.queues.fulfilment} /><Queue title="Invitation delivery attention" rows={report.queues.invitationDelivery} /></div>
...
function Queue({ title, rows }: { title: string; rows: Record<string, unknown>[] }) { return <section className="admin-report-section"><h2>{title}</h2>{rows.length ? <ol className="admin-report-queue">{rows.map((row, index) => <li key={String(row.orderId ?? row.invitationId ?? index)}><strong>{String(row.dropTitle ?? row.dropSlug ?? 'Design')}</strong><span>{row.shippingEmailStatus === 'failed' ? 'shipping email failed' : String(row.fulfilmentStatus ?? row.deliveryStatus ?? '')}</span><span>{String(row.destinationCountryCode ?? '')}</span></li>)}</ol> : <p className="admin-state">No items require attention.</p>}</section> }
