...
async function post<T>(url: string, token: string, payload: Record<string, unknown>): Promise<T> {
  ...
  const body = await response.json().catch(() => null) as { error?: { message?: string } } | null
  if (!response.ok) throw new AdminApiError(response.status, body?.error?.message ?? 'The operation could not be completed.')
  return body as T
}
...
export function runAdminAction(token: string, payload: Record<string, unknown>) { return post<{ success: true; delivery?: string; fulfilmentStatus?: string }>('/api/admin/actions', token, payload) }
