...
function Overview({ token }: { token: string }) { return <section aria-labelledby="overview-title"><p className="admin-kicker">Controlled workspace</p><h1 id="overview-title">Overview</h1><p className="admin-intro">Actions require explicit confirmation and server verification. Payment status cannot be changed here.</p><div className="admin-metrics">{(['reservations', 'invitations', 'orders', 'payments'] as const).map((resource) => <Metric key={resource} resource={resource} token={token} />)}</div><OperationForm token={token} /></section> }
function OperationForm({ token }: { token: string }) {
  const [reservationId, setReservationId] = useState(''); const [confirmation, setConfirmation] = useState(''); const [state, setState] = useState<'idle' | 'sending' | 'success' | 'error'>('idle'); const [message, setMessage] = useState('')
  const submit = async (event: FormEvent) => { event.preventDefault(); if (state === 'sending') return; setState('sending'); setMessage(''); try { const result = await runAdminAction(token, { action: 'invitation.prepare', reservationId, confirmation, idempotencyKey: crypto.randomUUID() }); setState('success'); setMessage(result.delivery === 'suppressed' ? 'Invitation prepared and delivery safely suppressed.' : 'Invitation sent.') } catch (error) { setState('error'); setMessage(error instanceof Error ? error.message : 'The invitation could not be prepared.') } }
  return <section className="admin-operation" aria-labelledby="invite-operation-title"><h2 id="invite-operation-title">Send one invitation</h2><p>Preview is available through the secured action API. Sending creates or rotates one personal invitation for one reservation; delivery is suppressed unless explicitly enabled in an approved environment.</p><form onSubmit={submit}><label>Reservation UUID<input value={reservationId} onChange={(event) => setReservationId(event.target.value)} required /></label><label>Type CONFIRM to prepare this invitation<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} required /></label><button type="submit" disabled={state === 'sending' || confirmation !== 'CONFIRM'}>{state === 'sending' ? 'Preparing…' : 'Prepare invitation'}</button></form>{message && <p className={`admin-message ${state === 'error' ? 'admin-error' : ''}`} role={state === 'error' ? 'alert' : 'status'}>{message}</p>}</section>
}
