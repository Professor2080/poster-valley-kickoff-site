    ...
    <section id="first-drop" className="section-pad bg-paper text-ink">
      <div className="mx-auto grid max-w-[82rem] gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <a
          ...
          aria-label={`Open detail page for ${drop.title}`}
        >
        ...
        <div className="max-w-2xl lg:pl-8">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            <span className="label-pill">{drop.editionLabel}</span>
            <span className="label-pill">{drop.format}</span>
            <span className="label-pill">{drop.priceLabel}</span>
            <span className="label-pill">Reservations open</span>
          </div>
          <p className="mt-8 text-xl leading-9 text-ink/64">
            Our first poster drop is being prepared as an {drop.format} release. Reserve your copy
            now; no payment is taken unless you later confirm a personal order invitation.
          </p>
          <p className="mt-5 text-sm leading-7 text-ink/48">
            Creator: {drop.creator}. Final paper, shipping timing and payment link are shared
            before your reservation becomes definitive.
          </p>
          <div className="mt-9 flex flex-col gap-4 sm:flex-row">
            <a className="button-dark" href={`${drop.href}#drop-interest`}>
              Reserve this poster
            </a>
            <a className="button-outline-dark" href={drop.href}>
              View details
            </a>
          </div>
        </div>
