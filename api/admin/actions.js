...
async function previewAction(admin, action, request) {
  if (action.startsWith('board.') || action.startsWith('drop.') || action.startsWith('delivery.')) {
    return adminRpc('admin_order_flow_preview_action', { p_actor: admin.userId, p_action: action, p_request: request })
  }
